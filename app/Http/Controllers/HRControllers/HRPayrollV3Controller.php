...
class HRPayrollV3Controller extends Controller
{
    ...
    //    return $request->all();
        ...
        //                 ->where('deleted', 0)
        //                 ->where('payrollid', $payrollperiod->id)
        //                 ->where('payrollid', $firstpayroll_cutoff->id)
        //                 ->get();
        ...
        //             if (isset($firstpayroll_cutoff)) { // kani if sa second cutoff of the month
        //                 // if ($paiddeduction->payrollid == $firstpayroll_cutoff->id) {
        //                 //     if ($paiddeduction->paidstatus == 1) {
        //                 //         // nabayaran siya sa first cutoff
        //                 //     } else {
        //                 //         //  wala nabayaran
        //                 //     }
        //                 // } else {
        //                 //     // wala nabayran
        //                 //     if ($paiddeduction->payrollid == 1) {
        //                 //         // nabayaran siya sa first cutoff
        //                 //     } else {
        //                 //         //  wala nabayaran
        //                 //     }
        ...
        //             $paiddeductions = DB::table('hr_payrollv2historydetail')
        ...
        //                     foreach ($paiddeductions as $paiddeduction) {
        ...
        if(count($otherdeductions)>0)
        {
            ...
            foreach($otherdeductions as $eachotherdeduction)
            {
                if ($eachotherdeduction->fullamount == $eachotherdeduction->totalotherdeductionpaid) {
                    ...
                    if ($formattedStartDate < $payrollperiod->datefrom || $formattedStartDate == $payrollperiod->datefrom || $formattedStartDate == $payrollperiod->dateto) {
                        ...
                        if ($cutoff == 2) {
                            ...
                            $headerid = DB::table('hr_payrollv2history')
                                ...
                                ->where(function ($query) use ($payrollperiod, $firstpayroll_cutoff) {
                                    $query->where('payrollid', $payrollperiod->id)
                                        ->orWhere('payrollid', $firstpayroll_cutoff->id);
                                })
                                ->get();
                            ...
                            if ($firstpayroll_cutoff) {
                                $firstpayol = DB::table('hr_payrollv2historydetail')
                                    ->select('hr_payrollv2historydetail.*')
                                    ->join('hr_payrollv2','hr_payrollv2historydetail.payrollid','=','hr_payrollv2.id')
                                    ->where('particulartype',2)
                                    ->where('hr_payrollv2.deleted','0')
                                    ...
                                    ->where('hr_payrollv2historydetail.deductionid', $eachotherdeduction->deductionotherid)
                                    ->first();
                                // return collect($firstpayol);   
                            }

                            if ($firstpayol) {
                                if ($firstpayol->paymenttype == 0) {
                                    $eachotherdeduction->paidforthismonth = 1;
                                    $eachotherdeduction->paidstatus = 0;
                                    $eachotherdeduction->paymenttype = 0;
                                } else if($firstpayol->paymenttype == 1) {
                                    $eachotherdeduction->paymenttype = 1;
                                }
                            } else {
                                
                                if (count($paid_deductions) == 0) {
                                    $eachotherdeduction->paymenttype = '';
                                }
        ...
        if ($employeecustomsched) {
            ...
            // $attendance = collect($attendance)->where('date', '2024-05-14')->values();
                ...
                if($checkifexists)
                {
                    ...
                    foreach ($employees as $employeeinfo) {
                        ...
                        // return collect($basicsalaryinfo);
                                ...
                                if (!empty($dates)) {
                                    ...
                                    if ($employeecustomsched) {
                                        ...
                                        if (count($attendance)>0) {
                                            ...
                                            // return $attendance;
                                            foreach ($attendance as $att) {
                                                    if (($att->amin != null || $att->amout != null || $att->pmin != null || $att->pmout != null) && ($att->timeinam != $att->timeinpm  || $att->timeoutpm != null)) {
                                                    // if (($att->amin != null || $att->amout != null || $att->pmin != null || $att->pmout != null) && $att->timeinpm == null) {
                                                    // for late
                            ...
                            // $otherdeductions = collect($otherdeductions_all)->where('employeeid', $employeeinfo->id)->where('description', 'DELP')->values();
                            ...
                            // return $otheraddedearningsparticularsamount;
                            ...
                            // }
                                ...
                                if(count($particulars) == 0)
                                {
                                    ...
                                    // $deletedparticulars = DB::table('hr_payrollv2historydetail')
                                    //     ->where('headerid', $checkhistoryifexists->id)
                                    //     // ->where('payrollid', $checkifexistsid)
                                    //     ->whereNull('particulartype')
                                    //     ->where('type', '0')
                                    //     ->where('deleted', 1)
                                    //     ->get();

                                    // return $deletedparticular;
                                    // return collect($allparts)->where('particulartype', $particular->particulartype)->where('particularid', $particular->particularid);
                                    // return 'eeee';
                                    // $notmatchingsdeductions = [];
                                    // $notmatchingodeductions = [];
                                    ...
                                    if(count($allparts) == 0)
                                    {
                                            ...
                                            if (property_exists($particular, 'paidstatus') && $particular->paidstatus == 1) {
                                                ...
                                                if(collect($allparts)->where('particulartype', $particular->particulartype)->where('particularid', $particular->particularid)->where('description', $particular->description)->count()>0)
                                                {
                                                    ...
                                                    // return collect($allparts)->where('particulartype', $particular->particulartype)->where('particularid', $particular->particularid)->first()->id;
                                                        ...
                                                        // if ($notreleasedid = collect($allparts)->where('particulartype', $particular->particulartype)->where('description', $particular->description)->where('particularid', '!=', $particular->particularid)->values()) {
                                                        //     $deductionpaidnotreleaseid = $notreleasedid->pluck('id');

                                                        //     DB::table('hr_payrollv2historydetail')
                                                        //         ->whereIn('id', $deductionpaidnotreleaseid)
                                                        //         ->update([
                                                        //             'paidstatus'       => 0,
                                                        //             'updatedby'             => auth()->user()->id,
                                                        //             'updateddatetime'       => date('Y-m-d H:i:s')
                                        ...
                                        foreach ($allpartsnotod as $particularnotexisted) {
                                            if ($particularnotexisted->particulartype === null && 
                                        ...
                                        // if (!$deletedparticulars->isEmpty()) {
                                        //     foreach ($deletedparticulars as $deletedparticular) {
                                        //         DB::table('hr_payrollv2addparticular')
                                        //             ->where('description', $deletedparticular->description)
                                        //             ->where('headerid', $checkhistoryifexists->id)
                                        //             ->where('payrollid', $checkifexistsid)
                                        //             ->where('employeeid', $employeeinfo->id)
                                        //             ->update([
                                        //                 'deleted' => 1,
                                        //                 'deleteddatetime' => Carbon::now()
                                        //             ]);
                                        //     }
                                        // }
                                        // return 'dddddd';
            ...
            foreach ($employees as $employeeinfo) {
                ...
                // return collect($basicsalaryinfo);
                    ...
                    foreach ($otherdeductions as $otherdeduction) {
                        ...
                        $payrolldetails = collect($payrolldetails_all)->where('employeeid', $employeeinfo->id)->where('particularid', $otherdeduction->id)->values();
                    ...
                    //             $paiddeductionItem = collect($paiddeduction_all)
                    //                 ->where('employeeid', $employeeinfo->id)
                    //                 ->where('particularid', $eachotherdeduction->id)
                    //                 ->where('payrollid', '<=', $payrollperiod->id)
                    //                 ->first();
    ...
    public function batchgenerateemployeepayroll(Request $request){
        ...
        // return $filteredGeneratedDataParticularsEarnings;
