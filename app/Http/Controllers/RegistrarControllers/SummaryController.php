        ...
        if(!$request->has('action'))
        {
            ...
            if($request->get('action') == 'updatesignatory')
            {
                ...
                }elseif($request->get('type') == 'sl')
                {
                    ...
                    $studentsByYearAndCourse = $students->groupBy(['coursename', 'levelid'])
                    ->map(function ($studentsByLevel, $courseName) {
                        return $studentsByLevel->map(function ($students, $levelId) use ($courseName) {
                            return (object)[
                                'courseDesc' => $students->first()->courseDesc,
                                'courseName' => $courseName,
                                'levelId' => $levelId,
                                'students' => $students->map(function ($student) {
                                    return (object)[
                                        'sid' => $student->sid ?? null,
                    ...
                    if($request->get('action') == 'filter')
                    {
                    ...
                    } else if($request->get('action') == 'excellstudentlist'){
                        $semester = DB::table('semester')
                            ->where('id', $request->get('semid'))
                            ->first()->semester;
                        ...
                        $drawing->setPath(public_path('schoollogo/schoollogo.png')); // Set your logo file path
                        $drawing->setCoordinates('A3'); // Position at the top left
                        $drawing->setOffsetX(20); // Adjust to position the logo properly
                        $drawing->setHeight(100); // Set height
                        $drawing->setWorksheet($sheet);

                        // Merge cells for the school name and title
                        $sheet->mergeCells('B3:D3');
                        $sheet->setCellValue('B3', DB::table('schoolinfo')->first()->schoolname);

                        // Retrieve the address and wrap it to a certain width
                        $address = DB::table('schoolinfo')->first()->address;
                        $newAddress = wordwrap($address, 60, PHP_EOL, true);

                        // Set the address to a merged cell and enable text wrapping
                        $sheet->mergeCells('B4:D4');
                        $sheet->setCellValue('B4', $newAddress);
                        ...
                        $sheet->setCellValue('B5', 'ENROLLMENT SUMMARY' .' - '. $semester.' '. $sydesc);
                        ...
                        $sheet->setCellValue('A' . $row, 'Student ID')
                            ...
                            ->setCellValue('C' . $row, 'Gender')
                            ->setCellValue('D' . $row, 'Units Enrolled');
                        $sheet->getStyle('A' . $row . ':D' . $row)->getFont()->setBold(true);
                        $sheet->getStyle('A' . $row . ':D' . $row)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
                        $sheet->getStyle('D' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);

                        // Set the header row to repeat on every printed page
                        $sheet->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(10, 10);

                        // Start adding data after the logo and title
                        $row++;

                        // Your loop code for adding students and totals remains the same
                        $grandTotal = 0;

                        foreach ($studentsByYearAndCourse as $studentsByCourse) {
                            $courseTotal = 0;
                            $sheet->mergeCells('A' . $row . ':D' . $row);
                            $sheet->setCellValue('A' . $row, $studentsByCourse[0]->courseDesc);
                            ...
                            foreach ($studentsByCourse as $courseName => $students) {
                                ...
                                // Determine the value based on levelId
                                if ($students->levelId == 17) {
                                    $cellValue = '1ST YEAR COLLEGE';
                                } elseif ($students->levelId == 18) {
                                    $cellValue = '2ND YEAR COLLEGE';
                                } elseif ($students->levelId == 19) {
                                    $cellValue = '3RD YEAR COLLEGE';
                                } elseif ($students->levelId == 20) {
                                    $cellValue = '4TH YEAR COLLEGE';
                                } else {
                                    $cellValue = '5TH YEAR COLLEGE'; // Default value
                                }
                                ...
                                $sheet->getStyle('A' . $row)->getFont()->setBold(true);
                                $row++;
                                // List each student under this course
                                foreach ($students->students as $student) {
                                    $sheet->setCellValue('A' . $row, $student->sid)
                                        ->setCellValue('B' . $row, $student->lastname . ', ' . $student->firstname)
                                        ->setCellValue('C' . $row, ucfirst($student->gender))
                                        ->setCellValue('D' . $row, $student->totalunits);

                                    $courseTotal++;
                                    $row++;
                                }

                                // Count the total number of students for this year level
                                $studentCount = count($students->students);

                                // Display the Year Level Count
                                $sheet->setCellValue('B' . $row, 'Year Level Count: ' . $studentCount);
                                $sheet->getStyle('B' . $row)->getFont()->setBold(true);
                                $sheet->getStyle('B' . $row )->getBorders()->getTop()->setBorderStyle(Border::BORDER_THIN);

                                $row++;
                            }
                            $grandTotal += $courseTotal;

                            // Display the Course Total
                            $sheet->setCellValue('A' . $row, 'Course Total: ' . $courseTotal);
                            $sheet->getStyle('A' . $row)->getFont()->setBold(true);
                            $row++;
                            $row++;
                        }

                        $sheet->setCellValue('A' . $row, 'GRAND TOTAL: ' . $grandTotal);
                        $sheet->getStyle('A' . $row)->getFont()->setBold(true);
                        ...
                        // Output Excel file
                        $filename = 'Enrollment_Summary_' . now()->format('Y-m-d_His') . '.xlsx';
    ...
    public function alphaloadingfilter(Request $request)
    {
        ...
        elseif(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'spct')
        {
            ...
            if($acadprogid == '6')
            {
                ...
                // return $students;
                $studsched = DB::table('college_studsched')
                    ->select('college_prospectus.id as prospectusid','college_studsched.studid','college_studsched.schedid')
                    ->whereIn('college_studsched.studid', collect($students)->pluck('id'))
                    ->join('college_classsched','college_studsched.schedid','college_classsched.id')
                    ->join('college_prospectus','college_classsched.subjectID','=','college_prospectus.id')
                    ->where('college_classsched.syID', $selectedschoolyear)
                    ...
                    if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'dcc')
                    {
                        ...
                        $classscheddetail_colleges = Db::table('college_classsched')
                            ->select('college_classsched.id','college_classsched.id as schedid','college_classsched.subjectID as subjectid','college_prospectus.subjCode as subjcode','college_prospectus.subjDesc as subjectname','college_prospectus.id as prospectusid','college_scheddetail.stime','college_scheddetail.etime','days.description as day','days.description','college_scheddetail.roomid','rooms.roomname',DB::raw('labunits + lecunits as units') ,'college_scheddetail.schedotherclass as term','college_classsched.teacherid','teacher.lastname','teacher.firstname','college_courses.courseDesc as deptcourse','college_courses.courseabrv','college_sections.sectionDesc as sectionname','college_sections.yearID as levelid','gradelevel.levelname','college_sections.id as sectionid','college_prospectus.courseID as courseid','college_schedgroup.schedgroupdesc as schedgroupdesc')
                            ->join('college_prospectus','college_classsched.subjectID','=','college_prospectus.id')
                            ->join('college_sections','college_classsched.sectionID','=','college_sections.id')
                            // ->join('college_year','college_sections.yearID','=','college_year.levelid')
                            ->leftJoin('gradelevel','college_sections.yearID','=','gradelevel.id')
                            ...
                            //     $join->on('college_courses.id', '=', 'college_schedgroup.courseid'); // Assuming the `courseid` is the correct column in `college_schedgroup`.
                            //     $join->where('college_schedgroup.deleted', 0);
                            //     $join->whereColumn('college_schedgroup.levelid', '=', 'college_sections.yearID');
                            // })
                            ->leftJoin('college_schedgroup', function ($join) {
                            ...
                            ->leftJoin('rooms','college_scheddetail.roomID','=','rooms.id')
                            ->where('college_scheddetail.deleted','0')
                            // ->where('college_prospectus.subjDesc', 'Literature and Humanities')
                            ->distinct('subjectid')
                            // ->groupBy('subjectid', 'schedgroupdesc')
                            ->orderBy('subjcode','asc')
                            ->get();
                    }
            ...
            if($request->has('export'))
            {
                ...
                elseif($request->get('exporttype') == 'pdfstudentsgs')
                {
                    ...
                    $grades = DB::table('college_studsched')
                        ...
                        ->leftJoin('college_studentprospectus','college_prospectus.id','=','college_studentprospectus.prospectusID')
                        ->select('subjCode as subjcode','subjDesc as subjdesc', 'labunits','lecunits','fg as subjgrade'
                ...
                $sched_array = collect($allschedules)->pluck('id');
                ...
                return view('registrar.summaries.alphaloadtable')
                    ...
                    ->with('collegeinstructors',$collegeinstructors)
                    ->with('teachers',$teachers)
                    ->with('sched_group_detail',$sched_group_detail)
                    ->with('allschedules',$allschedules);
                // }
            }
    }
