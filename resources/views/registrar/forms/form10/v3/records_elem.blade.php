        ...
        <div class="form-inline" style="display: unset !important;">
            <div class="btn-group">
                ...
                <div class="btn-group">
                    ...
                    <button class="btn-exportform-elem dropdown-item" data-exporttype="pdf" data-format="deped" data-layout="1"><i class="fa fa-file-pdf"></i> &nbsp;Deped (Template 1)</button>
                    <button class="btn-exportform-elem dropdown-item" data-exporttype="pdf" data-format="deped-2" data-layout="1"><i class="fa fa-file-pdf"></i> &nbsp;Deped (Template 2 - with school logo)</button>
                    @if($acadprogid == 5)
                    <button class="btn-exportform-elem dropdown-item" data-exporttype="pdf" data-format="depedspr" data-layout="1"><i class="fa fa-file-pdf"></i> &nbsp;Deped - SPR (Template 3)</button>
                    @endif
                    <button class="btn-exportform-elem dropdown-item" data-exporttype="pdf" data-format="school" data-layout="1"><i class="fa fa-file-pdf"></i> &nbsp;School Template @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'hccsi')1 @endif<br/><small><em class="text-danger">Note: Please provide the school's own template</em></small></button>
                    @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'hccsi')
                    <button class="btn-exportform-elem dropdown-item" @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'dcc')data-exporttype="excel"@else data-exporttype="pdf"@endif data-format="school" data-layout="2"><i class="fa fa-file-pdf"></i> &nbsp;School Template 2<br/><small><em class="text-danger">Note: Please provide the school's own template</em></small></button>
                    @endif
                    </div>
                </div>
        </div>
            ...
            <div class="card-body">
                <div class="tab-content">
                    <div class="tab-pane active" id="tab_eligibility">   
                        ...
                        <div class="row mt-1">
                            <div class="col-12 text-right">
                                <button type="button" class="btn btn-sm btn-primary" id="btn-eligibility-update-elem"><i class="fa fa-edit"></i> Update</button>
                            </div>
                        </div>
                    </div>
                    @foreach ($gradelevels as $key=>$gradelevel)
                        <div class="tab-pane  {{--@if($key == 0) active @endif--}}" id="tab_{{$key}}">                            
                                @if(collect($records)->where('levelid', $gradelevel->id)->count() == 0)             
                                    <fieldset class="form-group border p-2 fieldset-grades">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <table class="table table-striped" style="width: 100%; font-size: 11px; table-layout: fixed;">
                                                        ...
                                                        @if(count($gradelevel->subjects)>0)
                                                            @foreach($gradelevel->subjects as $defaultsubject)
                                                                <tr class="eachsubject">
                                                                    ...
                                                                    <td colspan="2" style="vertical-align: middle;"><button type="button" class="btn btn-sm btn-block btn-default p-0 btn-deleterow"><small><i class="fa fa-trash-alt"></i></small></button></td>
                                                                </tr>
                                                            @endforeach
                                                        @endif
                                                    </tbody>
                                ...
                                <form action="/schoolform10/updatesigneachsem" method="POST" class="form-eachlevel" data-levelid="{{$gradelevel->id}}" data-sydesc="{{$eachrecord->sydesc}}">
                                    @csrf
                                    <div class="row eachsem-other-container">
                                        <div class="col-md-12">
                                            <label>Remarks</label>
                                            <textarea class="form-control" name="remarks">@if(isset($eachrecord)){{(collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->count() > 0 ? collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->first()->remarks : '') ?? ''}}@endif</textarea>
                                        </div>
                                        <div class="col-md-4">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" class="form-control form-control-sm input-certncorrectname text-center" name="certncorrectname" @if(isset($eachrecord))value="{{(collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->count() > 0 ? collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->first()->certncorrectname : '') ?? ''}}"@endif/>
                                            <input type="text" class="form-control form-control-sm input-certncorrectdesc text-center" name="certncorrectdesc" @if(isset($eachrecord))value="{{(collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->count() > 0 ? collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->first()->certncorrectdesc : "SHS-School Record's In-Charge") ?? "SHS-School Record's In-Charge"}}"@else value="SHS-School Record's In-Charge" @endif/>
                                        </div>
                                        <div class="col-md-4 text-right">
                                            <small><label>Date Checked (MM/DD/YYYY)</label></small>
                                            <input type="date" class="form-control form-control-sm input-datechecked" name="datechecked" @if(isset($eachrecord))value="{{(collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->count() > 0 ? collect($eachlevelsignatories)->where('levelid', $eachrecord->levelid)->first()->datechecked : '') ?? ''}}"@endif/>
                                                            ...
                                                            <thead class="text-center">
                                                                <tr>
                                                                    ...
                                                                    <th colspan="3" class="text-right">
                                                                        <button type="button" class="btn btn-outline-primary btn-sm btn-edit-editremedialheader" data-studid="{{$studinfo->id}}" data-sydesc="{{$eachrecord->sydesc}}"  data-action="header" data-levelid="{{$eachrecord->levelid}}"><i class="fa fa-share"></i> Update</button>
                                                                    </th>
                                                                </tr>
                                                                <tr class="text-uppercase">
                                                                    <th>Learning Areas</th>
                                                            ...
                                                            <tfoot>
                                                                <tr>
                                                                    <td style="width: 15%;" class="text-right">
                                                                        <button type="button" class="btn btn-default btn-sm btn-block btn-remedial-addrow" data-studid="{{$studinfo->id}}" data-sydesc="{{$eachrecord->sydesc}}" data-levelid="{{$eachrecord->levelid}}"><i class="fa fa-plus"></i> Subject</button>
                                                                    </td>
                                                                    <td colspan="5">
                                                                        &nbsp;
                                                                    </td>
    ...
    $('.btn-edit-editremedialheader').on('click', function(){
        ...
        $('.btn-remedial-addrow').on('click', function(){
            ...
            $(this).closest('table').find('#remedial-tbody').append(
                ...
                <td class="text-right"><button type="button" class="btn btn-sm btn-outline-success p-1 btn-edit-addremedial" id="btn-edit-addremedial`+levelid+`-`+counter+`" data-studid="`+studentid+`" data-sydesc="`+sydesc+`" data-action="add" data-levelid="`+levelid+`"><i class="fa fa-edit"></i> Save</button> <button type="button" class="btn btn-sm btn-outline-danger p-1  removebutton"><i class="fa fa-trash-alt text-danger"></i>&nbsp;</button></td>
                </tr>
                `
            );
        ...
        $('.btn-exportform-elem').on('click', function(){
            ...
            window.open('/schoolform10/getrecordselem?export='+formexport+'&format='+format+'&exporttype='+exporttype+'&studentid='+studentid+'&acadprogid='+acadprogid+'&layout='+layout+'&papersize='+papersize,'_blank')
        })
    })
