    ...
    <div class="col-md-12 text-right">
        ...
        <div class="btn-group">
            ...
            <div class="dropdown-menu dropdown-menu-right" style="font-size: 14px;">
                <button class="btn-export-record dropdown-item" data-exporttype="pdf" data-format="school" data-layout="1" id="btn-exportpdf"><i class="fa fa-file-pdf"></i> &nbsp;PDF -  
                    @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'mci')Application for Graduation from Collegiate Course @else Record of Candidate For Graduation @endif</button>
            </div>
          </div>
    </div>
</div>
    ...
    <div class="card-header p-1">
        <div class="row">
            ...
            <div class="col-md-6"> 
                ...
                <input type="date" class="form-control form-control-sm" id="input-graduationdate"/>
            </div>
            @else
            <div class="col-md-6"> {{--sbc--}}
                <label>Title/Degree</label>
                <input type="text" class="form-control form-control-sm" placeholder="Ex. Diploma In Midwifery" id="input-degree" value="{{$degree != null ? $degree : $coursename}}"/>
            </div>
            @endif
            <div class="col-md-6">
                <label>Entrance Data</label>
                <input type="text" class="form-control form-control-sm" placeholder="Ex. Certificate of Transfer Credential/Transcript of Records for Evaluation" id="input-entrancedata" value="{{$details->entrancedata ?? ''}}"/>
            </div>
            @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'sbc')
            <div class="col-md-4">
                <label>Intermediate Course</label>
                <input type="text" class="form-control form-control-sm" placeholder="" id="input-intermediatecourse" value="{{$details->secondcourse ?? ''}}"/>
            </div>
            <div class="col-md-2">
                <label>Year</label>
                <input type="text" class="form-control form-control-sm" placeholder="" id="input-intermediateyear" value="{{$details->intermediatesy ?? ''}}"/>
            </div>
            <div class="col-md-4">
                <label>Secondary Course</label>
                <input type="text" class="form-control form-control-sm" placeholder="" id="input-secondarycourse" value="{{$details->intermediatecourse ?? ''}}"/>
            </div>
            <div class="col-md-2">
                <label>Year</label>
                <input type="text" class="form-control form-control-sm" placeholder="" id="input-secondaryyear" value="{{$details->secondsy ?? ''}}"/>
            </div>
            @endif
            <div class="col-md-3">
                        ...
                        @foreach($collectgradelevel->subjects as $eachsubject)
                            <tr>
                                ...
                                @if(count($subjgroups)>0)
                                    ...
                                    @endforeach
                                @endif
                                <td><button type="button" class="btn btn-sm btn-default p-0 btn-block btn-updateunitplot" data-syid="{{$collectgradelevel->syid}}" data-semid="{{$collectgradelevel->semid}}" data-subjectid="{{$eachsubject['subjectID']}}">Update</button></td>
                            </tr>
                        @endforeach
                        <tr>
                            <td>
