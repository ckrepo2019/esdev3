...
<html lang="en">
   ...
   @page { margin: 40px 20px 5px 20px; size: {{$papersize != null ? $papersize : '8.5in 13in'}};}/*8.5 13*/
    ...
    @else --}}
        <table style="width: 100%; top: -0.4cm; position: absolute;" id="table1">
            <tr>
                ...
                <td width="10%" style="text-align:right;" @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'hc babak') rowspan="6" @else rowspan="5" @endif><img src="{{base_path()}}/public/assets/images/deped_logo.png" alt="school" width="80px"></td>
            </tr>
            <tr>
...
<table class="table table-sm table-bordered" width="100%" style="font-size: 10px !important; margin-top:.2rem !important; table-layout: fixed;">
    <tr>
        <td width="2%" class="text-center" style="vertical-align: top;">
            <input type="checkbox" style="border: 1px solid black;"@if($eligibility->peptpasser == '1') checked @endif/>
        </td>
        ...
        <td width="2%" style="vertical-align: top;" class="text-center">
            <input type="checkbox" style="border: 1px solid black;"@if($eligibility->alspasser == '1') checked @endif/>
        </td>
        ...
        <td width="2%"></td>
        <td width="2%" style="vertical-align: top;" class="text-center">
            <input type="checkbox" style="border: 1px solid black;"@if(strlen($eligibility->others) > 0) checked @endif/>
        </td>
...
@foreach($gradelevels as $eachlevelkey=>$eachgradelevel)
    @foreach ($eachgradelevel->records as $eachrecordkey=>$eachrecord)
        ...
        <table style="width: 100%; table-layout: fixed; font-size: 10px;">
                    ...
                    <td colspan="6">
                        <table style="width: 100%; font-size: 10px;">
                            <tr>
                                ...
                                <td style="width: 8%; border-bottom: 1px solid black;">{{preg_replace('/\D+/', '', $eachrecord->levelname)}}</td>
                                <td style="width: 2%;">SY:</td>
                                <td style="width: 13%; border-bottom: 1px solid black;">{{$eachrecord->sydesc}}</td>
                                <td style="width: 4%;">SEM:</td>
                                <td style="width: 6%; border-bottom: 1px solid black;">@if($eachrecord->semid == 1) 1st @elseif($eachrecord->semid == 2) 2nd @endif</td>
                            </tr>
                        </table>
                        <table style="width: 100%; font-size: 10px;">
                            <tr>
                                <td style="width: 10%;">TRACK/STRAND:</td>
                                <td style="width: 60%; border-bottom: 1px solid black;">{{$eachrecord->trackname}} / {{$eachrecord->strandname}}</td>
            ...
            <tbody  style="border-left: 2px solid black; border-right: 2px solid black; border-bottom: 2px solid black">
                ...
                @else
                    ...
                    $defaultsubjects = collect($eachgradelevel->subjects)->where('semid', $eachrecord->semid)->unique('subjcode');
                    @endphp
                    @for($x=0; $x<10; $x++)
                        <tr >
        ...
        @php
            ...
            $certncorrectdesc = '';
            $datechecked = '';
            if(collect($eachsemsignatories)->where('levelid', $eachrecord->levelid)->where('semid',$eachrecord->semid)->first())
            {
                $signinfo = collect($eachsemsignatories)->where('levelid', $eachrecord->levelid)->where('semid',$eachrecord->semid)->first();
                ...
                $datechecked = $eachrecord->datechecked != null ?  date('M d, Y',strtotime($eachrecord->datechecked)) : '';
            }
        @endphp
        ...
        <table style="width: 100%; table-layout: fixed; font-size: 10px;">
            ...
            <tr>
                <td style="width: 30%; border-bottom: 1px solid black; text-align: center;">&nbsp;{{$eachrecord->teachername}}</td>
                <td style="width: 5%;"></td>
        ...
        <table style="width: 100%; table-layout: fixed; font-size: 9px; page-break-inside: avoid;">
            @if(collect($eachrecord->remedials)->contains('type','2'))
                @php
                    $eachsemremedial_header = collect($eachrecord->remedials)->where('type','2')->values();
