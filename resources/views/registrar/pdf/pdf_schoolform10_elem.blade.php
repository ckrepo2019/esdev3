<style>
    * { font-family: Arial, Helvetica, sans-serif; }
    @page { margin: 20px; size: {{$papersize != null ? $papersize : '8.5in 14in'}};} /*8.5 14*/
...
<table style="width: 100%" id="table1">
    <tr>
        ...
        <td width="15%" style="text-align:right;" rowspan="5"><img src="{{base_path()}}/public/assets/images/deped_logo.png" alt="school" width="70px"></td>
    </tr>
    <tr>
...
<table style="width: 100%" id="table1">
    ...
    <tr>
        <td></td>
        <td style="text-align:center; font-size: 11px;">Division of {{DB::table('schoolinfo')->first()->divisiontext}}</td>
    </tr>
    <tr>
        <td></td>
    ...
    <tr>
        <td></td>
        <td style="text-align:center; font-size: 12px; font-weight: bold;">Learner's Permanent Academic Record for Elementary School (SF10-ES)</td>
    </tr>
                                    ...
                                    <td colspan="7">
                                        ...
                                        <table style="width: 100%;">
                                            <tr>
                                                ...
                                                <td style="width: 10%; border-bottom: 1px solid black;">@if(strpos(strtolower($record[0]->levelname),'grade') !== false){{preg_replace('/\D+/', '', $record[0]->levelname)}}@else {{$record[0]->levelname}} @endif</td>
                                ...
                                @foreach($record[0]->grades as $grade)
                                    @if($record[0]->type == 2)
                                        @if(strtolower($grade->subjtitle) != 'general average')
                                            <tr>
                                                ...
                                                <td class="text-center">@if($grade->finalrating > 0){{$grade->finalrating}}@endif</td>
                                    ...
                                    @else
                                        <tr>
                                            ...
                                            <td class="text-center">@if($grade->finalrating > 0){{number_format($grade->finalrating)}}@endif</td>
                                            <td class="text-center">@if($grade->finalrating > 0){{$grade->remarks != null ? $grade->remarks : ($grade->finalrating > 74 ? 'PASSED' : 'FAILED')}}@endif</td>
                                        </tr>
                                    @endif
                                @endforeach
                                ...
                                @if($record[0]->type == 1)
                                    @if(strtolower(DB::table('schoolinfo')->first()->schoolid) == '405308') {{--fmcma--}}
                                        ...
                                        <td class="text-center">{{collect($record[0]->generalaverage)->first()->finalrating}}</td>
                                            <td class="text-center">@if(collect($record[0]->generalaverage)->count()>0){{collect($record[0]->generalaverage)->first()->finalrating >= 75 ? 'PASSED' : 'FAILED'}}@endif</td>
                                        </tr>
                                    @else
                                        <tr style="font-weight: bold;">
                                            ...
                                            <td class="text-center">{{number_format(collect($record[0]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[0]->grades)->where('inMAPEH','0')->count()) > 0 ? number_format(collect($record[0]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[0]->grades)->where('inMAPEH','0')->count()) : ''}}</td>
                                            <td class="text-center">{{number_format(collect($record[0]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[0]->grades)->where('inMAPEH','0')->count()) == 0 ? '':(number_format(collect($record[0]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[0]->grades)->where('inMAPEH','0')->count()) >=75 ? 'PASSED':'FAILED')}}</td>
                                        </tr>
                                    @endif
                            ...
                            <table style="width: 100%; table-layout: fixed; border: 2px solid black;" border="1">
                                ...
                                @if(count($record[1]->grades)>0)
                                    ...
                                    @if($record[1]->type == 1)
                                        ...
                                        <tr style="font-weight: bold;">
                                            <td>&nbsp;&nbsp;General Average</td>
                                            <td class="text-center">@if(collect($record[1]->generalaverage)->count()>0){{collect($record[1]->generalaverage)->first()->quarter1}}@endif</td>
                                            <td class="text-center">@if(collect($record[1]->generalaverage)->count()>0){{collect($record[1]->generalaverage)->first()->quarter2}}@endif</td>
                                            <td class="text-center">@if(collect($record[1]->generalaverage)->count()>0){{collect($record[1]->generalaverage)->first()->quarter3}}@endif</td>
                                            <td class="text-center">@if(collect($record[1]->generalaverage)->count()>0){{collect($record[1]->generalaverage)->first()->quarter4}}@endif</td>
                                           <td class="text-center">{{collect($record[1]->generalaverage)->first()->finalrating}}</td>
                                            <td class="text-center">@if(collect($record[1]->generalaverage)->count()>0){{collect($record[1]->generalaverage)->first()->finalrating >= 75 ? 'PASSED' : 'FAILED'}}@endif</td>
                                        </tr>
                                        @else
                                            <tr style="font-weight: bold;">
                                                ...
                                                <td class="text-center">{{number_format(collect($record[1]->grades)->sum('finalrating')/count($record[1]->grades)) > 0 ? number_format(collect($record[1]->grades)->sum('finalrating')/count($record[1]->grades)) : ''}}</td>
                                                <td class="text-center">{{number_format(collect($record[1]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[1]->grades)->where('inMAPEH','0')->count()) == 0 ? '':(number_format(collect($record[1]->grades)->where('inMAPEH','0')->sum('finalrating')/collect($record[1]->grades)->where('inMAPEH','0')->count()) >=75 ? 'PASSED':'FAILED')}}</td>
                                            </tr>
                                        @endif
                                    @elseif($record[1]->type == 2)
                                        ...
                                        @if(collect($generalaverage)->count() > 0)
                                                <tr style="font-weight: bold;">
                                                    <td colspan="5"  style="text-align: right;">&nbsp;&nbsp;General Average</td>
                                                    <td class="text-center">{{collect($generalaverage)->first()->finalrating}}</td>
                                                    <td class="text-center">{{collect($generalaverage)->first()->finalrating >= 75? 'PASSED':'FAILED'}}</td>
                                                </tr>
                                        @else
                                        <tr style="font-weight: bold;">
                                            <td colspan="5"  style="text-align: right;">&nbsp;&nbsp;General Average</td>
