    ...
    <div class="row">
        @if ($locksf2 == 0)
            ...
            <div class="col-md-6 text-right">
                <button type="button" class="btn btn-primary save-button" id="btn-save"><i class="fa fa-share"></i> Save
                    Changes</button>
                    ...
                    @if (count($dates) > 0)
                        @foreach ($dates as $date)
                            <th class="text-center eachdate" data-date="{{ $date->date }}"
                                ...
                                {{ $date->datestr }}<br />
                                {{ $date->day }}<br />
                                <select class="form-control form-control-sm select-column-att"
                                    data-date="{{ $date->date }}">
                                ...
                                <td style="vertical-align: middle;">
                                    <div class="row">
                                        <div class="col-md-4 align-self-center">
                                            <select class="form-control form-control-sm select-row-att"
                                                data-id="{{ $student->id }}">
                                                <option value="reset"></option>
                                                <option value="reset">Reset</option>
                                                <option value="present">Present</option>
                                ...
                                @if (count($student->attendance) > 0)
                                    @foreach ($student->attendance as $att)
                                        <td data-class="attstatus"
                                            ...
                                            data-studid="{{ $student->id }}" data-tdate="{{ $att->tdate }}"
                                            @if ($att->status == null) data-status="PRESENT" 
                                 ...
                                 @else 
                                      ...
                                      clicked="0" @endif>
                                            ...
                                            @elseif($att->status == 'AM PRESENT')
                                                <span class="badge badge-success">AM</span> <span
                                                    class="badge badge-success">PRESENT</span>
                                            @elseif($att->status == 'PM PRESENT')
                                                <span class="badge badge-secondary">PM</span> <span
                                                    class="badge badge-success">PRESENT</span>
                                            ...
                                            @elseif($att->status == 'AM LATE')
                                                <span class="badge badge-success">AM</span> <span
                                                    class="badge badge-warning">LATE</span>
                                            @elseif($att->status == 'PM LATE')
                                                <span class="badge badge-secondary">PM</span> <span
                                                    class="badge badge-warning">LATE</span>
                                            @elseif($att->status == 'AM CC')
                                                <span class="badge badge-success">AM</span> <span
                                                    class="badge badge-secondary">CC</span>
                                            ...
                                            <br>
                                            @if ($att->ttime != null)
                                                <span class="badge timebadge" style="font-size:.7rem !important"
        ...
        <table class="table-head-fixed text-nowrap table-bordered" id="thetable" style="font-size: 12px;">
            <thead>
                <tr>
                    ...
                    @if (count($dates) > 0)
                        @foreach ($dates as $date)
                            <th class="text-center" data-date="{{ $date->date }}"
                                style="width: 100px !important;">
                                {{ $date->datestr }}<br />
                                {{ $date->day }}
                            </th>
